#!/bin/bash
# ============================================================
#  PublicDrop API — Ultra-Fast Upgrade v3
#  SAFE: Never deletes coins.db, live_prices.json, logos,
#        or your existing API keys in live_prices.py
#
#  Run:
#  bash <(curl -s https://raw.githubusercontent.com/VoidProJect1/PublicDrop.in/main/upgrade_publicdrop_v3.sh)
# ============================================================
...
echo "[1/5] Installing flask-compress..."
sudo python3 -m pip install flask-compress -q 2>/dev/null || true
echo "✓ flask-compress ready"

# ── Step 2: Backup existing API keys ─────────────────────
echo "[2/5] Saving your existing API keys..."
SAVED_KEYS=$(python3 -c "
import re
content = open('$INSTALL_DIR/live_prices.py').read()
keys = re.findall(r'\"(CG-[^\"]+)\"', content)
print('\n'.join(keys))
" 2>/dev/null || echo "")
KEY_COUNT=$(echo "$SAVED_KEYS" | grep -c "CG-" 2>/dev/null || echo "0")
echo "✓ $KEY_COUNT CoinGecko key(s) found and saved"

# ── Step 3: Write new live_prices.py ─────────────────────
echo "[3/5] Upgrading live_prices.py..."
...
def fetch_global():
    ...
    while True:
        try:
            ...
            h   = {"Accept": "application/json", "User-Agent": "PublicDrop/2.0"}
            if key: h["x-cg-demo-api-key"] = key
            gm  = requests.get("https://api.coingecko.com/api/v3/global", headers=h, timeout=15).json()
            gd  = gm.get("data", {})
            with _lock:
            ...
            with _global_lock: _global.update(data)
            with open(GLOBAL_FILE, "w") as fp: json.dump(data, fp)
            log.info(f"[GLOBAL] F&G={fg_value}({fg_label}) | Vol=${data['total_volume_24h_usd']:,.0f} | G={gainers} L={losers}")
        except Exception as e:
...
def load_existing():
    try:
        if os.path.exists(LIVE_FILE):
            ...
            log.info(f"Loaded {len(_prices)} existing prices from disk")
    except Exception as e:
        log.error(f"Load existing failed: {e}")
...
# ── Restore API keys into the new file ───────────────────
if [ -n "$SAVED_KEYS" ] && [ "$KEY_COUNT" -gt "0" ]; then
    python3 << KEYEOF
import re

keys = [k for k in """$SAVED_KEYS""".strip().split('\n') if k.startswith('CG-')]
f = "$INSTALL_DIR/live_prices.py"
content = open(f).read()

if keys:
    lines = ',\n    '.join([f'"{k}"' for k in keys])
    new_block = f'API_KEYS = [\n    {lines},\n]'
    content = re.sub(r'API_KEYS = \["PLACEHOLDER"\]', new_block, content)
    open(f, "w").write(content)
    print(f"✓ {len(keys)} API key(s) restored: {', '.join([k[:12]+'...' for k in keys])}")
else:
    print("⚠ No keys restored — add them manually to live_prices.py")
KEYEOF
else
    echo "⚠ No keys found — add them manually: nano $INSTALL_DIR/live_prices.py"
fi

# ── Step 4: Patch api_server.py ──────────────────────────
echo "[4/5] Patching api_server.py..."
python3 << 'PATCHEOF'
f = "/opt/publicdrop-api/api_server.py"
content = open(f).read()
applied = 0

# PATCH 1: Compression import
if "flask_compress" not in content:
    ...
    print("· Compression already present")

# PATCH 2: Cache block after app = Flask(__name__)
CACHE_BLOCK = """
# ── Compression ──────────────────────────────────────────
if _HAS_COMPRESS:
    Compress(app)
...
def _build_cache():
    ...
    print(f"[CACHE] Built — {len(tmp)} entries from {len(rows)} coins")
...
# PATCH 3: Build cache on startup
if "_build_cache()" not in content:
    content = content.replace(
        "    init_db()\n    # Start background scheduler",
        "    init_db()\n    _build_cache()  # Load all coins into RAM\n    # Start background scheduler"
    )
    applied += 1
    print("✓ Cache built on startup")
else:
    print("· Startup cache already present")

# PATCH 4: Ultra-fast details endpoint
OLD_DETAILS = """@app.route('/APIv3/details')
def details():
...
NEW_DETAILS = """@app.route('/APIv3/details')
def details():
    sym = request.args.get('sym') or request.args.get('id') or ''
    d = _find_cached(sym)
    if not d:
...
elif "_find_cached" in content:
    print("· Details already ultra-fast")
else:
    print("⚠ Details endpoint — pattern not matched (check manually)")

# PATCH 5: Ultra-fast search
OLD_SEARCH = """@app.route('/APIv3/search')
def search():
    ...
    c = conn.cursor()
    rows = c.execute(\"\"\"
        SELECT id, symbol, name, rank, logo_url, logo_local
        FROM coins
...
NEW_SEARCH = """@app.route('/APIv3/search')
def search():
    q = (request.args.get('q') or '').strip().lower()
    if len(q) < 1:
        return jsonify({"error": "Query too short"}), 400
...
elif "seen, results = set()" in content:
    print("· Search already ultra-fast")
else:
    print("⚠ Search endpoint — pattern not matched (check manually)")

# PATCH 6: Upgraded status endpoint
OLD_STATUS = """@app.route('/APIv3/status')
def status():
    conn = get_db()
    c = conn.cursor()
    ...
    with_logo = c.execute(f"SELECT COUNT(*) FROM coins WHERE logo_local IS NOT NULL").fetchone()[0]
    conn.close()
    return jsonify({
        "total_coins": total,
...
if OLD_STATUS in content:
    ...
    applied += 1
    print("✓ Status endpoint upgraded")
elif "fear_greed_value" in content:
    print("· Status already upgraded")
else:
    print("⚠ Status endpoint — pattern not matched (check manually)")
...
echo ""
echo " Data Safety:"
echo "   ✓ coins.db         — UNTOUCHED"
echo "   ✓ live_prices.json — UNTOUCHED"
echo "   ✓ logos/           — UNTOUCHED"
echo "   ✓ API keys         — RESTORED automatically"
echo ""
echo " New Features:"
echo "   ⚡ RAM cache        0.1ms response time"
echo "   😨 Fear & Greed    every 8 hours"
echo "   📊 24h Volume       every 8 hours"
echo "   📈 Gainers/Losers   live in /status"
echo "   🗜  Gzip compress   smaller responses"
echo ""
echo " Test:"
