...
if not BACKUP.exists():
    ...
    print("Backup saved to " + str(BACKUP))
else:
    print("Backup already exists at " + str(BACKUP))
...
RATE_LIMIT_FILE = _Path("/opt/publicdrop-api/data/rate_limits.json")
STATS_FILE      = _Path("/opt/publicdrop-api/data/admin_stats.json")
...
# Guard: only patch if app is already defined at this point
if 'app' not in dir():
    raise RuntimeError("install_admin: 'app' Flask object not found at injection point. "
                       "Move the # -- Entrypoint -- marker to after app = Flask(...) in the server file.")
...
def _sys_stats():
    try:
        ...
        logo_dir = _Path("/opt/publicdrop-api/data/logos")
...
def admin_access():
    ...
    err_html = ("<div class=\\"err\\">&#x26A0; " + err + "</div>") if err else ""
...
<div class="wrap">
  ...
  <div class="card">
    <form method="POST">
      ...
      <input type="password" name="pw" placeholder="&bull;&bull;&bull;&bull;&bull;&bull;&bull;&bull;" autofocus>
      """ + err_html + """
      <button class="btn" type="submit">Access Dashboard &rarr;</button>
    </form>
  </div>
  <div class="footer">publicdrop.in &middot; Restricted Access</div>
</div>
</body>
</html>"""
...
def admin_dashboard():
    ...
    sys_info  = _sys_stats()
    uptime    = _uptime_str(_time.time() - _launch_time)
    meta_svc  = _svc_status("publicdrop-meta")
    live_svc  = _svc_status("publicdrop-live")
    api_svc   = _svc_status("publicdrop-apiv3")

    # Safe access to host-server globals _meta / _live
    _g = globals()
    _meta_len = len(_g["_meta"]) if "_meta" in _g else 0
    _live_len = len(_g["_live"]) if "_live" in _g else 0
    ...
    meta_path = Path("/opt/publicdrop-api/data/meta.json")
    live_path = Path("/opt/publicdrop-api/data/live.json")
    meta_size = meta_path.stat().st_size // 1024 if meta_path.exists() else 0
    live_size = live_path.stat().st_size // 1024 if live_path.exists() else 0
    ...
    for ip2, cfg2 in _rate_cfg.items():
        ...
        note    = cfg2.get("note", "")
        if blocked:
            badge = \'<span class="badge badge-dn">Blocked</span>\'
        elif premium:
            badge = \'<span class="badge badge-up">Premium</span>\'
        else:
            badge = \'<span class="badge badge-neu">Custom</span>\'
        unblock_label = "Unblock" if blocked else "Block"
        toggle_val    = "false" if blocked else "true"
        rate_rows += (
            "<tr>"
            "<td class=\\"mono\\">" + ip2 + "</td>"
            "<td>" + badge + "</td>"
            "<td class=\\"mono\\">" + str(lim) + "/min</td>"
            "<td class=\\"t3\\">" + (note or "&mdash;") + "</td>"
            "<td>"
            "<button class=\\"tbtn tbtn-dn\\" onclick=\\"doBlock(\'" + ip2 + "\'," + toggle_val + ")\\">" + unblock_label + "</button>"
    ...
    top_ip_rows = "".join(
        "<tr><td class=\'mono\'>" + ip2 + "</td><td class=\'mono\'>" + str(cnt) + "</td></tr>"
        for ip2, cnt in top_ips
    )
    top_ep_rows = "".join(
        "<tr><td class=\'mono\'>" + ep + "</td><td class=\'mono\'>" + str(cnt) + "</td></tr>"
        for ep, cnt in top_eps
    )

    def svc_badge(s):
        if s == "active":
            return \'<span class="badge badge-up">&#x25CF;&nbsp;Active</span>\'
        return \'<span class="badge badge-dn">&#x25CF;&nbsp;\' + s + \'</span>\'

    cpu_pct      = sys_info.get("cpu_pct", 0)
    ...
    disk_used    = sys_info.get("disk_used_mb", 0)
    logo_count   = sys_info.get("logo_count", 0)

    cpu_color  = "var(--dn)" if cpu_pct > 80  else "var(--accent)"
    ram_color  = "var(--dn)" if ram_pct > 85  else "var(--up)"
    disk_color = "var(--dn)" if disk_pct > 85 else "var(--gold)"

    no_rules_row   = "<tr><td colspan=\'5\' class=\'t3\' style=\'text-align:center;padding:20px;\'>No custom IP rules yet</td></tr>"
    no_ip_row      = "<tr><td colspan=\'2\' class=\'t3\' style=\'text-align:center;padding:16px;\'>No data yet</td></tr>"
    no_ep_row      = "<tr><td colspan=\'2\' class=\'t3\' style=\'text-align:center;padding:16px;\'>No data yet</td></tr>"
...
<div class="main">
  ...
  <div class="grid grid-3">
    <div class="card">
      ...
      <div class="card-val">""" + "{:.1f}".format(cpu_pct) + """<span style="font-size:16px;color:var(--t2)">%</span></div>
      <div class="pbar-wrap">
        <div class="pbar-bg"><div class="pbar-fill" style="width:""" + "{:.1f}".format(cpu_pct) + """%;background:""" + cpu_color + """;"></div></div>
        <div class="pbar-label">Oracle Linux 9 &middot; Always Free VM</div>
      </div>
    </div>
    <div class="card">
      <div class="card-label">RAM Usage</div>
      <div class="card-val">""" + "{:.1f}".format(ram_pct) + """<span style="font-size:16px;color:var(--t2)">%</span></div>
      <div class="pbar-wrap">
        <div class="pbar-bg"><div class="pbar-fill" style="width:""" + "{:.1f}".format(ram_pct) + """%;background:""" + ram_color + """;"></div></div>
        <div class="pbar-label">""" + "{:,}".format(ram_used) + """ MB / """ + "{:,}".format(ram_total) + """ MB</div>
      </div>
    </div>
    <div class="card">
      <div class="card-label">Storage Usage</div>
      <div class="card-val">""" + "{:.1f}".format(disk_pct) + """<span style="font-size:16px;color:var(--t2)">%</span></div>
      <div class="pbar-wrap">
        <div class="pbar-bg"><div class="pbar-fill" style="width:""" + "{:.1f}".format(disk_pct) + """%;background:""" + disk_color + """;"></div></div>
        <div class="pbar-label">""" + "{:,}".format(disk_used) + """ MB used &middot; """ + "{:,}".format(logo_count) + """ logos &middot; meta """ + str(meta_size) + """ KB &middot; live """ + str(live_size) + """ KB</div>
      </div>
    </div>
  </div>
  ...
  <div class="svc-grid">
    <div class="svc-card">
      <div>
        ...
        <div class="svc-desc">Gunicorn &middot; Port 7780</div>
      </div>
  ...
  <div class="ctrl-card" style="margin-bottom:12px;">
    <div class="ctrl-row">
      ...
      <input class="ctrl-input" id="GLOBAL_LIM" type="number" value=\"""" + str(_GLOBAL_LIMIT) + """\" min="1" max="10000">
  ...
  <div class="tbl-wrap">
    <table>
      ...
      <tbody id="RATE_TBODY">
        """ + (rate_rows if rate_rows else no_rules_row) + """
      </tbody>
    </table>
  </div>
  ...
  <div class="grid grid-2">
    <div class="tbl-wrap">
      <table>
        ...
        <tbody>""" + (top_ip_rows if top_ip_rows else no_ip_row) + """</tbody>
      </table>
    </div>
    <div class="tbl-wrap">
      <table>
        <thead><tr><th>Top Endpoints</th><th>Hits</th></tr></thead>
        <tbody>""" + (top_ep_rows if top_ep_rows else no_ep_row) + """</tbody>
      </table>
    </div>
  </div>
...
async function setGlobalLimit() {
  var lim = parseInt(document.getElementById(\'GLOBAL_LIM\').value);
  if (!lim || lim < 1) { toast(\'Enter a valid limit\', false); return; }
  var r = await api(\'/APIv3/admin/api/global-limit\', {limit: lim});
  if (r.ok) { toast(\'Global limit set to \' + lim + \'/min\'); } else { toast(\'Error: \' + r.error, false); }
}

async function addPremium() {
  var ip   = document.getElementById(\'NEW_IP\').value.trim();
  var lim  = parseInt(document.getElementById(\'NEW_LIM\').value) || 0;
  var note = document.getElementById(\'NEW_NOTE\').value.trim();
  if (!ip) { toast(\'Enter an IP address\', false); return; }
  var r = await api(\'/APIv3/admin/api/ip-rule\', {ip: ip, limit: lim || null, premium: true, note: note});
  if (r.ok) { toast(\'Premium added: \' + ip); setTimeout(function(){ location.reload(); }, 1000); }
...
async function addBlock() {
  ...
  var r = await api(\'/APIv3/admin/api/ip-rule\', {ip: ip, blocked: true, note: note});
  if (r.ok) { toast(\'Blocked: \' + ip); setTimeout(function(){ location.reload(); }, 1000); }
  else { toast(\'Error: \' + r.error, false); }
}

async function doBlock(ip, block) {
  var r = await api(\'/APIv3/admin/api/ip-rule\', {ip: ip, blocked: block});
  var msg = block ? (\'Blocked: \' + ip) : (\'Unblocked: \' + ip);
  if (r.ok) { toast(msg); setTimeout(function(){ location.reload(); }, 1000); }
  else { toast(\'Error: \' + r.error, false); }
}

async function setLimit(ip) {
  var lim = prompt(\'New req/min limit for \' + ip + \':\');
  if (!lim) return;
  var r = await api(\'/APIv3/admin/api/ip-rule\', {ip: ip, limit: parseInt(lim), premium: true});
  if (r.ok) { toast(\'Limit updated: \' + ip + \' -> \' + lim + \'/min\'); setTimeout(function(){ location.reload(); }, 1000); }
  else { toast(\'Error: \' + r.error, false); }
}

async function removeIP(ip) {
  if (!confirm(\'Remove rule for \' + ip + \'?\')) return;
  var r = await api(\'/APIv3/admin/api/remove-ip\', {ip: ip});
  if (r.ok) { toast(\'Removed: \' + ip); setTimeout(function(){ location.reload(); }, 1000); }
  else { toast(\'Error: \' + r.error, false); }
}

async function svcAction(action, svc) {
  if (action === \'stop\' && !confirm(\'Stop \' + svc + \'? API calls will fail.\')) return;
  var r = await api(\'/APIv3/admin/api/service\', {action: action, service: svc});
  if (r.ok) { toast(svc + \' \' + action + \'ed\'); setTimeout(function(){ location.reload(); }, 2000); }
  else { toast(\'Error: \' + r.error, false); }
}
...
# Inject admin block before entrypoint
content = TARGET.read_text()

MARKER = '# -- Entrypoint --'
if MARKER not in content:
    # Try alternative marker formats
    for alt in ['# ── Entrypoint ──', '# -- Entrypoint --', '# Entrypoint']:
        if alt in content:
            MARKER = alt
            break
    else:
...
if "admin_access" in content:
    print("Admin block already injected. Re-injecting cleanly...")
    start = content.find("\n# ============================================================================\n# ADMIN DASHBOARD")
    end   = content.find("\n# END ADMIN DASHBOARD")
