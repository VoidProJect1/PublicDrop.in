#!/bin/bash
# ============================================================
# PublicDrop Crypto API — Full Setup Script
# Fetches 5000+ coins from CoinGecko, stores permanently,
# serves as publicdrop.in/APIv3/
# ============================================================
...
echo "======================================"
echo " PublicDrop Crypto API Installer"
echo "======================================"
...
#!/usr/bin/env python3
"""
PublicDrop Crypto API Server
Endpoints:
  /APIv3/details?sym=BTC
  /APIv3/details?sym=bitcoin
  /APIv3/logo?sym=BTC
  /APIv3/coins          -> all coins list
  /APIv3/search?q=btc
  /APIv3/categories?cat=defi
  /APIv3/status
"""

import os, json, time, threading, requests, sqlite3, hashlib
...
HEADERS    = {"Accept": "application/json", "User-Agent": "PublicDrop/1.0"}
...
TW_CHAIN_MAP = {
    ...
    "FLOW":"flow","EOS":"eos","TON":"ton","TRX":"tron","APT":"aptos","SUI":"sui",
    "OP":"optimism","ARB":"arbitrum","LINK":"ethereum","UNI":"ethereum",
    "AAVE":"ethereum","THETA":"theta","ONE":"harmony",
}
...
def init_db():
    ...
    """)
    # Basic coin list table (from /coins/markets, no rate issues)
    c.execute("""
...
# ── Logo Fetcher ─────────────────────────────────────────
def get_logo_urls(sym, contracts=None):
    s = sym.lower()
    urls = []
    # 1. TrustWallet - coin (layer 1)
    ...
    # 2. TrustWallet - token (via contract address)
    if contracts:
        try:
...
def cg_get(endpoint, params=None, retries=3):
    """CoinGecko GET with retry and rate limit handling"""
    ...
    for attempt in range(retries):
        try:
            ...
            if r.status_code == 200:
                ...
                wait = 65
                print(f"[CG] Rate limited, waiting {wait}s...")
                time.sleep(wait)
            else:
                time.sleep(5)
...
def fetch_coin_list():
    """Fetch all coins list from CoinGecko (no rate limit concern)"""
...
def fetch_markets_batch():
    """Fetch market data for top 5000 coins (monthly updates)"""
    print("[FETCH] Fetching market data (monthly)...")
    conn = get_db()
    c = conn.cursor()
    for page in range(1, 21):  # 20 pages × 250 = 5000 coins
        ...
        if not data:
            break
        for coin in data:
            # Upsert basic + monthly data
            c.execute("""
                INSERT INTO coins (id, symbol, name, rank, max_supply, total_supply,
                    ath_usd, ath_date, atl_usd, atl_date, logo_url, monthly_updated_at)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
...
def fetch_permanent_details(cg_id, sym):
    ...
    # Download logo
    logo_local = download_logo(sym, cg_id, contracts)

    conn = get_db()
    c = conn.cursor()
    c.execute("""
        UPDATE coins SET
            description=?, logo_local=?,
            website=?, whitepaper=?,
...
def fetch_all_permanent():
    """Fetch permanent details for all coins that don't have it yet"""
    conn = get_db()
    c = conn.cursor()
    rows = c.execute("""
    ...
    for i, row in enumerate(rows):
        ...
        time.sleep(2.2)  # Stay under 30/min limit
...
def background_scheduler():
    ...
    # Start permanent details fetch in background
    threading.Thread(target=fetch_all_permanent, daemon=True).start()

    while True:
        now = datetime.utcnow()
        # Monthly update at midnight on 1st of month
        if now.day == 1 and now.hour == 0:
            print("[SCHEDULER] Monthly update starting...")
            fetch_markets_batch()
            time.sleep(3700)  # Skip rest of hour
...
def logo():
    ...
    d = dict(row)
    # Return local file if downloaded
    if d.get('logo_local') and os.path.exists(d['logo_local'].replace('/assets/logos', LOGO_DIR).replace(LOGO_DIR, LOGO_DIR)):
        local_path = f"{LOGO_DIR}/{d['symbol'].lower()}.png"
        if os.path.exists(local_path):
            return send_file(local_path, mimetype='image/png')
    # Return best URL
    urls = get_logo_urls(d['symbol'], d.get('contracts'))
...
def all_coins():
    ...
    c = conn.cursor()
    rows = c.execute("""
        SELECT id, symbol, name, rank, ath_usd, logo_url, logo_local,
...
def categories():
    ...
    c = conn.cursor()
    rows = c.execute("""
        SELECT id, symbol, name, rank, logo_url
...
[Unit]
Description=PublicDrop Crypto API
...
# ── 6. Start the API ─────────────────────────────────────
echo "[6/6] Starting PublicDrop API..."
...
echo ""
echo " Fetching is running in background..."
